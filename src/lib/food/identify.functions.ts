...
export interface ProductIdentificatie {
  ...
  uitleg: string;
  triggers: string[];
  alternatieven: string[]; // Nieuw veld voor de alternatieven-module
}

const SYSTEM_PROMPT = `Je bent de specialistische analyse-engine van "Sulfiet & Lever Coach". Jouw cliënten hebben een overbelast SUOX-enzym (zwavel/sulfiet) en een uitgeput SULT-pad (Lever-Fase-2). Je negeert histamine- of IgE-allergieën volledig.
...
Geef ALTIJD antwoord in een puur JSON-object met exact deze structuur:
{
  ...
  "alternatieven": ["veilig alternatief 1", "veilig alternatief 2"]
}

RICHTLIJN VOOR ALTERNATIEVEN:
Als een product ORANJE of ROOD is, geef dan 1 tot 3 biochemisch logische alternatieven die wél VEILIG (GROEN) zijn voor zowel het SUOX- als het SULT-pad.
- Bijv. bij Aardbei: ["Blauwe bessen", "Frambozen (met mate)", "Peer (geschild)"]
- Bijv. bij Paracetamol: ["Magnesium (Tauraat of Bisglycinaat)", "Palmitoylethanolamide (PEA)", "Paracetamol + NAC (Glutathion-buffer)"]
- Bijv. bij Knoflook: ["Bieslook (vers, met mate)", "Gember", "Asafetida (Hing)"]
Als het product al GROEN is, mag de lijst leeg zijn of suggesties geven voor vergelijkbare veilige producten.
...
1. SUOX -> Verwerkt sulfieten en zwavel (Wijn, gedroogd fruit, ui, knoflook, prei, sjalot, broccoli, bloemkool, spruitjes, rucola).
...
const respSchema = z.object({
  ...
  alternatieven: z.array(z.string()).default([]), // Validatie voor het nieuwe veld
});
...
export const identificeerProduct = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<ProductIdentificatie> => {
    const userPrompt = `De gebruiker zoekt: "${data.query}".${data.ingredienten ? `\nIngrediëntenlijst: ${data.ingredienten}` : ""}\nAnalyseer op basis van SUOX en SULT, geef veilige alternatieven indien nodig, en lever de gevraagde JSON op.`;
